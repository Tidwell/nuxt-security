...
You can toggle CSP to report-only mode to monitor violations without blocking any content. This is useful for testing CSP policies before enforcing them.
...
When the `contentSecurityPolicyReportOnly: true` togggle is set, the `Content-Security-Policy-Report-Only` header will be used instead of `Content-Security-Policy` with any existing `headers.contentSecurityPolicy` configuration.

If you need to simultaniously serve Report-Only and existing CSP headers, you can define them individually.
  ...
  security: {
    headers: {
      ...
      contentSecurityPolicyReportOnly: {
        'script-src': ["'strict-dynamic'"], // test removing 'self'
        'report-uri': ['/api/csp-report']
      }
    }
  }
